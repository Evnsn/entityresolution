...
class AttributeEquivalenceBlocking(BaseBlocking):
    ...
    def build(self, df_a, df_b=None, return_as="df"):
        ...
        if df_b is None: # Deduplication
            ...
            df_a = df_a.dropna(subset=[self.left_on]).copy() # Remove NaN (allow_nan parameter?)
            df_b = df_b.dropna(subset=[self.right_on]).copy() # Remove NaN (allow_nan parameter?)
            ...
            merged_df[f"AE_{self.left_on}"] = merged_df.apply(lambda row: row[self.left_on], axis=1) # Add block column
